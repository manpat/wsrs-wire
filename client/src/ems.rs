...
pub fn start(ctx_ptr: *mut MainContext) {
	...
	unsafe {
		let input_state = &mut (*ctx_ptr).input_state as *mut InputState;
		emscripten_set_mousedown_callback(ptr::null(), input_state as *mut u8, 0, on_mouse_down);
		emscripten_set_mouseup_callback(ptr::null(), input_state as *mut u8, 0, on_mouse_up);
		emscripten_set_mousemove_callback(ptr::null(), input_state as *mut u8, 0, on_mouse_move);

		emscripten_set_keydown_callback(ptr::null(), input_state as *mut u8, 0, on_key_down);
		emscripten_set_keyup_callback(ptr::null(), input_state as *mut u8, 0, on_key_up);

		emscripten_set_pointerlockchange_callback(ptr::null(), input_state as *mut u8, 0, on_pointer_lock_change);

		// emscripten_set_touchstart_callback(ptr::null(), input_state as *mut u8, 0, on_touch_down);
		// emscripten_set_touchend_callback(ptr::null(), input_state as *mut u8, 0, on_touch_up);
		// emscripten_set_touchmove_callback(ptr::null(), input_state as *mut u8, 0, on_touch_move);
		// emscripten_set_touchcancel_callback(ptr::null(), input_state as *mut u8, 0, on_touch_up);

		emscripten_set_main_loop_arg(on_update, ctx_ptr as *mut u8, 0, 1);
	}
}

pub fn activate_pointer_lock() {
	unsafe {
		emscripten_request_pointerlock(ptr::null(), 1);
	}
}
...
extern fn on_mouse_move(_: i32, ev: *const EmscriptenMouseEvent, ud: *mut u8) -> i32 {
	...
	let ev = unsafe { &*ev };
	ctx.on_mouse_move(ev.x, ev.y,  ev.dx, ev.dy);

	1
}

extern fn on_key_down(_: i32, ev: *const EmscriptenKeyboardEvent, ud: *mut u8) -> i32 {
	...
	if ev.key[1] == 0 {
		let c = (ev.key[0] as char).to_ascii_lowercase();
		if c.is_ascii_graphic() || c.is_ascii_whitespace() {
			ctx.on_key_down(Button::Ascii(c));
		}

	} else {
		let sl = ev.key.split(|c| *c == 0).next().unwrap();
		let name = str::from_utf8(&sl);
...
extern fn on_key_up(_: i32, ev: *const EmscriptenKeyboardEvent, ud: *mut u8) -> i32 {
	use std::ascii::AsciiExt;
	use std::str;
	use input::Button;
